...
<div class="container">
	<h2 class="title">Olah Data Satuan</h2>

	<div class="table-header">
		<div class="left-toolbar">
			<Button href="/dashboard/satuan-barang/tambah">
				<Plus class="icon" />
				<span>Data Baru</span>
			</Button>
		</div>
	...
	<div class="table-wrapper">
		<Table.Root>
			<Table.Header>
				<Table.Row>
					<Table.Head>ID Satuan</Table.Head>
			...
			<Table.Body>
				{#if data.satuanBarang.length > 0}
					{#each data.satuanBarang.filter((item) => item.namaSatuan
							.toLowerCase()
							.includes(search.toLowerCase())) as item (item.id)}
						<Table.Row>
							<Table.Cell>{item.idSatuan}</Table.Cell>
							<Table.Cell>{item.namaSatuan}</Table.Cell>
								...
								{#if data.session.role === 'ADMIN'}
									<div class="actions">
										<Button
											size="sm"
											variant="outline"
											href={`/dashboard/satuan-barang/edit?id=${item.id}`}
										>
											Edit
										</Button>
