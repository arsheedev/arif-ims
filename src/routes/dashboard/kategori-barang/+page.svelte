...
<div class="container">
	<h2 class="title">Olah Data Kategori</h2>

	<div class="table-header">
		<div class="left-toolbar">
			<Button href="/dashboard/kategori-barang/tambah">
				<Plus class="icon" />
				<span>Data Baru</span>
			</Button>
		</div>
		...
		<Table.Root>
			<Table.Header>
				<Table.Row>
					...
					<Table.Head>ID Kategori</Table.Head>
			...
			<Table.Body>
				{#if data.kategoriBarang.length > 0}
					{#each data.kategoriBarang.filter((item) => item.namaKategori
							.toLowerCase()
							.includes(search.toLowerCase())) as item (item.id)}
						<Table.Row>
							<Table.Cell>{item.namaKategori}</Table.Cell>
							<Table.Cell>{item.idKategori}</Table.Cell>
								...
								{#if data.session.role === 'ADMIN'}
									<div class="actions">
										<Button
											size="sm"
											variant="outline"
											href={`/dashboard/kategori-barang/edit?id=${item.id}`}
										>
											Edit
										</Button>
